...
# Wrap the key
RESULT=$(./psec-tool tr31 wrap -k "$KBPK" -H "$HEADER" "$KEY")
echo "wrapped: $RESULT"
...
RESULT2=$(./psec-tool tr31 wrap -k "$KBPK" -m 32 -H "$HEADER" "$KEY")
...
set +eo pipefail
./psec-tool tr31 wrap -k "$KBPK" -H "invalid" "$KEY" 2>&1 | grep -q "error:"
if [ $? -ne 0 ]; then
