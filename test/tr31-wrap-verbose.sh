...
for ver in A B C; do
  ...
  RESULT=$(./psec-tool tr31 wrap-verbose -k "$KBPK_TDES" "$header" "$KEY" 2>/dev/null)
...
for ver in D E; do
  ...
  RESULT=$(./psec-tool tr31 wrap-verbose -k "$KBPK_AES" "$header" "$KEY" 2>/dev/null)
  echo "$RESULT" | grep -q "Self-unwrap:.*(OK)" || { echo "FAIL: V$ver self-unwrap not OK"; exit 1; }
  KEY_BLOCK=$(echo "$RESULT" | grep "^  ${ver}" | tail -1 | tr -d ' ')
...
RESULT_M=$(./psec-tool tr31 wrap-verbose -k "$KBPK_TDES" -m 32 "C0016P0TE00N0000" "$KEY" 2>/dev/null)
...
set +eo pipefail
./psec-tool tr31 wrap-verbose -k "$KBPK_TDES" "X0016P0TE00N0000" "$KEY" 2>&1 | grep -q "Version ID.*not supported"
if [ $? -ne 0 ]; then
...
set +eo pipefail
./psec-tool tr31 wrap-verbose -k "$KBPK_TDES" "invalid" "$KEY" 2>&1 | grep -q "error:"
if [ $? -ne 0 ]; then
